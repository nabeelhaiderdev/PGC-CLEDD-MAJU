...
if ( ! empty( $_SERVER['SCRIPT_FILENAME'] ) && 'comments.php' == basename( $_SERVER['SCRIPT_FILENAME'] ) ) { /*1*/
		die( __( 'Please do not load this page directly. Thanks!', 'cledd_td' ) );
}
if ( post_password_required() ) { /*2*/ ?>
	<?php _e( 'This post is password protected. Enter the password to view comments.', 'cledd_td' ); ?>
	<?php
	return;
}
	...
	<div id="comments" class="comments-area">
		...
		<h3 class="section-title" id="comments">
			...
			if ( '1' === $theme_comment_count ) {
				printf(
					/* translators: 1: title. */
					__( 'One Responce', 'cledd_td' ),
					'<span>' . get_the_title() . '</span>'
				);
			} else {
				printf( // WPCS: XSS OK.
					/* translators: 1: comment count number, 2: title. */
					esc_html( _nx( '%1$s Responce', '%1$s Responces', $theme_comment_count, 'comments title', 'cledd_td' ) ),
		...
		<?php } else { // this is displayed if there are no comments so far ?>
			...
			if ( ! comments_open() ) {
				?>
		<p class="no-comments"><?php _e( 'Comments are closed.', 'cledd_td' ); ?></p>
				<?php
			}
